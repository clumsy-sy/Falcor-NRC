...
{
    float loss;
    if (mCudaResources.train_query == nullptr || mCudaResources.train_query_cnt == nullptr || mCudaResources.train_sample == nullptr ||
        mCudaResources.train_sample_cnt == nullptr)
    {
...
void NRCInterface::inference(uint32_t infer_cnt, bool useRF)
{
    nrc_network_ref->nrc_inference(
        mCudaResources.screen_query, mCudaResources.infer_cnt, mCudaResources.infer_query_pixel, mCudaResources.screen_result, useRF
    );
}
...
)
{
    ...
    mCudaResources.screen_query = (NRC::inputBase*)pScreenQueryBuffer.get()->getCudaMemory()->getMappedData();
    mCudaResources.train_query = (NRC::inputBase*)pTrainingQueryBuffer.get()->getCudaMemory()->getMappedData();
    mCudaResources.train_sample = (NRC::trainSample*)pTrainingSampleBuffer.get()->getCudaMemory()->getMappedData();
